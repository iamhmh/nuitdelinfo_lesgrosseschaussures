...
export class UIScene extends Phaser.Scene {
  ...
  private createStatsPanel(): void {
    ...
    const recondIcon = this.add.text(15, startY + lineHeight, '🐧', { fontSize: '18px' })
    this.reconditionedText = this.add.text(45, startY + lineHeight, 'Reconditionnés: 0', {
      fontSize: '14px',
      color: '#94a3b8',
    })
    
    const distIcon = this.add.text(15, startY + lineHeight * 2, '🏫', { fontSize: '18px' })
    this.distributedText = this.add.text(45, startY + lineHeight * 2, 'Distribués: 0 / 8', {
      fontSize: '14px',
      color: '#94a3b8',
    })
    
    const invIcon = this.add.text(15, startY + lineHeight * 3, '🎒', { fontSize: '18px' })
    this.inventoryText = this.add.text(45, startY + lineHeight * 3, 'Inventaire: 0 PC', {
      fontSize: '14px',
      color: '#fbbf24',
    })
    ...
    this.statsPanel = this.add.container(20, 20, [
      bg, title,
      collectedIcon, this.collectedText,
      recondIcon, this.reconditionedText,
      distIcon, this.distributedText,
      invIcon, this.inventoryText,
    ])
  }
  ...
  private createInteractHint(): void {
    ...
    // Texte
    const text = this.add.text(0, 0, '[ E ] Interagir', {
      fontSize: '16px',
      color: '#0f172a',
      fontStyle: 'bold',
    }).setOrigin(0.5)
  ...
  public showVictory(): void {
    ...
    // Animation d'entrée
    const container = this.add.container(0, 0, [overlay, victoryTitle, victoryText, button])
